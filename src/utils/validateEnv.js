...
export function validateMailersendKey(key) {
  ...
  if (!key.startsWith('mlsn_')) {
    const error = 'MAILERSEND_API_KEY must start with "mlsn_" - this appears to be an invalid MailerSend API key';
...
export function validateFirebaseServiceKey(keyJson) {
  ...
  try {
    ...
    if (!key.client_email.includes('@') || !key.client_email.includes('.iam.gserviceaccount.com')) {
      throw new Error('Invalid client_email format - must be a valid service account email');
    }
...
export function validateImageKitUrl(url) {
  ...
  if (!url) {
    if (isProduction) {
      throw new Error('[PRODUCTION] NEXT_PUBLIC_IMAGEKIT_URL_ENDPOINT is required');
    }
    return false;
  }
